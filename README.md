...
| `JWT_SECRET` | обязательна для Go | секрет подписи JWT длиной не менее 32 байт; в Docker при пустом значении генерируется и сохраняется в volume |
| `JWT_EXPIRY_MINUTES` | `60` | положительное число минут жизни JWT |
| `REQUEST_TIMEOUT_SECONDS` | `10` | максимальное время обработки HTTP-запроса, не более 300 секунд |
| `MIGRATION_TIMEOUT_SECONDS` | `60` | максимальное время запуска и ожидания lock миграций |
...
`GET /health` — liveness-проверка HTTP-сервера. `GET /ready` проверяет доступность PostgreSQL и используется healthcheck приложения в Compose. Тело JSON-запроса ограничено 1 MiB. Обработка запроса по умолчанию ограничена 10 секундами; transport timeout и graceful shutdown получают ещё 5 секунд для записи ответа и завершения.
...
`PUT` заменяет редактируемые поля целиком, `PATCH` меняет только переданные. Пустой `{}` в PATCH — успешная операция без изменения данных. `event_date` передаётся и возвращается как `YYYY-MM-DD`; технические timestamps — в RFC3339. Приоритет по умолчанию — 3; допустимые значения — 1–5. Ссылка подарка должна быть абсолютным HTTP(S) URL без userinfo. Пустые списки возвращаются как `[]`.

Ошибки имеют вид `{"error":"..."}`: `400` — malformed JSON, `401` — нет действительного JWT, `403` — чужой список, `404` — ресурс отсутствует, `409` — email занят или подарок уже забронирован, `413` — тело больше 1 MiB, `422` — JSON разобран, но поля невалидны, `504` — превышен deadline обработки. Внутренние ошибки возвращают `500` без деталей БД.
